...
//V0.2：增加了附加信息ID 0xE9，当参数0xF283设置为1时才会增加此附加信息ID,dxl
		...
		if(1 == Link2LastSendFlag)
		{
                    		...
                    		||((0 == Link2ReportAckFlag)&&(Link2ReportDistanceSpace >= 100)))//上一条位置信息汇报是否收到应答，1表示收到
			{
				...
				//{
					if(DetectStatus(DetectOption_functionTest))
					{
					Link2ReportBufferLen = 28;//只记录基本位置信息,701iii是Link1ReportBufferLen由liamtsen修改
					}
					else
					{
...
u16 Report_GetPositionBasicInfo(u8 *pBuffer)
{
	...
	if(option&&(0 == NavigationFlag))//dxl,2016.5.17 不定位时，bit18-bit21全为0
	{
	    temp &= 0xffc3ffff;
	}
	Public_ConvertLongToBuffer(temp,p);
...
static u16 Report_GetPositionAdditionalInfo(u8 *pBuffer)
{
	...
	if(DetectMultiStatus(0,2,DetectOption_functionTest,DetectOption_protocolTest))//dxl,2016.5.7部标检测时自定义的附加ID不加入
	{
