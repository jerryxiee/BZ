...
u32 TcpAckOvertime = 0;//TCP消息应答超时时间，对应参数ID 0x0002，这两个参数仅用于鉴权指令重发，主要是兼容行标检测
u32 TcpResendNum = 0;//TCP消息重传次数，对应参数ID 0x0003
//****************宏定义******************
////add by zwb 2017.9.12 start.....
DetectControlDef	DetectControl;
#include <stdarg.h>
...
//中断资源  :  
//返回      :0:开关未打开;1:开关打开
//备注      :DetectOption_gbTest:为1是国标检测模式
//备注      :DetectOption_protocolTest:为1是部标协议检测模式，当为协议检测时必须设置是几号机
//备注      :DetectOption_functionTest:为1是部标功能检测模式
//备注      :DetectOption_hyTest:为1是全国货运平台检测模式
//备注      :DetectOption_machine3:为1是部标协议检测3号机检测模式
//备注      :DetectOption_machine2:为1是部标协议检测2号机检测模式
//备注      :DetectOption_machine1:为1是部标协议检测1号机检测模式
//备注      :DetectOption_driverNoSignIn:为1是开启行驶时驾驶员未签到提醒
*********************************************************************/
u8 DetectStatus(E_DetectOption eStatus)
{
	u8 sta;

	sta = 0x1 << eStatus;
	
	return (DetectControl.test & sta);

}
/*********************************************************************
//函数名称  :DetectMultiStatus
//功能      :检测开关状态
//输入      :mode:0-->任一开关打开1-->选定的开关都打开  num:选定开关的数量
//输出      :
//使用资源  :
//全局变量  :   
//调用函数  :
...
//备注      :DetectOption_driverNoSignIn:为1是开启行驶时驾驶员未签到提醒
*********************************************************************/
u8 DetectMultiStatus(u8 mode,u8 num,...)
{
	u8 sta,flg = 0;
	E_DetectOption e;
	va_list args;
	...
	while(num--)
	{
		e = va_arg(args, E_DetectOption);
		sta = 0x1 << e;
		if(DetectControl.test & sta)
		{
			if(0 == mode)
			{
				return 1;
...
void UpdateRunMode(void)
{
	...
	{
		DetectControl.test = Buffer[0];
	}
	else
	{
		DetectControl.test = 0;
	}
}
////add by zwb 2017.9.12 end.....
...
FunctionalState DelayTrigTimeTask(void)
{
        ...
        case 0x0800://上发多媒体事件
              {
                    RadioProtocol_MultiMediaEvenReport(channel,PhotoID, MediaType, MediaFormat, MediaEvenCode, MediaChannelID);
                    break;
              }
        ...
        case 0x8801://接收到立即拍照指令后发送拍照应答指令,只应答1张图片
              {
                    ...
                    if(0 == MediaChannelID)
                    {
                          ...
                          if(i == (CAMERA_ID_MAX+1))
                          {
                                ...
                                Buffer[4] = 1;
                                //多媒体ID
                                Buffer[5] = (PhotoID&0xff000000) >> 24;
                                Buffer[6] = (PhotoID&0xff0000) >> 16;
                                Buffer[7] = (PhotoID&0xff00) >> 8;
                                Buffer[8] = PhotoID&0xff;
...
//		SOC_IPVAL ip_value;
		...
		if(DetectStatus(DetectOption_functionTest))//测794时只需要终端通用应答
		{
			switch(state)
			{
				case 0://初始化
...
/*******************************************************************************
* Function Name  : RadioProtocolStartRecord(void)
* Description    : 下发录音开始指令,录20秒后上传至监控中心
* Input          : None
* Output         : None
* Return         : 仅仅为行标北斗测试添加
*******************************************************************************/
void RadioProtocolStartRecord(void)
{
...
//中断资源	:  
//返回		:
//备注		:主要是TCP超时时间，TCP重传次数dxl,2015.6.11增加，主要是兼容行标检测要求
*********************************************************************/
void LoginUpdatePram(void)
{
         u8  Buffer[5];
        u8  BufferLen;
       //dxl,2015.6.11增加，主要是兼容行标检测要求
        if(0 == TcpAckOvertime)
        {
                BufferLen = EepromPram_ReadPram(E2_TCP_ACK_OVERTIME_ID,Buffer);
                if(BufferLen == E2_TCP_ACK_OVERTIME_LEN)
                {
                        TcpAckOvertime = Public_ConvertBufferToLong(Buffer);
                        if(TcpAckOvertime > 60)
                        {
                                TcpAckOvertime = 60;
                        }
                        else if(TcpAckOvertime < 5)
                        {
                                TcpAckOvertime = 20;//货运平台检测为5秒
                        }
                }
                else
                {
                        TcpAckOvertime = 20;//行标检测为20秒
                }
        }
        if(0 == TcpResendNum)
        {
                BufferLen = EepromPram_ReadPram(E2_TCP_RESEND_TIMES_ID,Buffer);
                if(BufferLen == E2_TCP_RESEND_TIMES_LEN)
                {
                         TcpResendNum = Public_ConvertBufferToLong(Buffer);
                        if( TcpResendNum > 10)
                        {
                ...
                {
                        TcpResendNum = 2;//行标检测为2次，共鉴权3次，第2次与第1次间隔20秒，第3次与第2次间隔40秒
                }
        } 
}
/*******************************************************************************
...
FunctionalState  Link1Login_TimeTask(void)
{
	u8  ucTmp[30];
        ...
        LoginUpdatePram();//dxl,2015.6.11增加，主要是兼容行标检测要求
	    ...
	    case LOGIN_SEND_AUTHORIZATION_CMD://发送鉴权命令
		    {
			    if(ACK_OK == RadioProtocol_TerminalQualify(CHANNEL_DATA_1))
                {
                    ...
                    //Link1LoginAuthorizationDelayTime = 30;//注册鉴权延时时间,给1秒做网络延时,dxl,2015.6.11,屏蔽
                                Link1LoginAuthorizationDelayTime = TcpAckOvertime*Link1AuthorizationCount - 1;//dxl,2015.6.11,为兼容行标送检而修改
			        if((Link1LoginAuthorizationDelayTime > 180)||(Link1LoginAuthorizationDelayTime < 5))//增加最大值和最小值限制
                                {
                                        Link1LoginAuthorizationDelayTime = 180;
                                }
                                Link1LoginAuthorizationLastState = LOGIN_SEND_AUTHORIZATION_CMD;//上一次状态
			        
                    //if(Link1AuthorizationCount >= 3)//dxl,2015.6.11,屏蔽
                                if(Link1AuthorizationCount > TcpResendNum)//dxl,2015.6.11,为兼容行标送检而修改
			        {
				        Link1LoginAuthorizationState = LOGIN_SEND_LOGIN_CMD;
			        }
			        else
	    ...
	    case LOGIN_CLOSE_GPRS://断开网络
		    {
                ...
                ClearTerminalAuthorizationCode(CHANNEL_DATA_1);//dxl,2015.6.11之前写的通道2是笔误,现在更正为通道1
	    ...
	    case LOGIN_SEND_AUTHORIZATION_CMD://发送鉴权命令
		    {
					if(ACK_OK == RadioProtocol_TerminalQualify(CHANNEL_DATA_2))
					{
						...
						Link2LoginAuthorizationDelayCount = 0;
						//Link2LoginAuthorizationDelayTime = 30;//注册鉴权延时时间,给1秒做网络延时,dxl,2015.6.11,为兼容行标送检而修改
						Link2LoginAuthorizationDelayTime = TcpAckOvertime*Link2AuthorizationCount - 1;//dxl,2015.6.11,为兼容行标送检而修改
						if((Link2LoginAuthorizationDelayTime > 180)||(Link2LoginAuthorizationDelayTime < 5))//增加最大值和最小值限制
						{
							Link2LoginAuthorizationDelayTime = 180;
						}
						Link2LoginAuthorizationLastState = LOGIN_SEND_AUTHORIZATION_CMD;//上一次状态

						//if(Link2AuthorizationCount >= 3)
						if(Link2AuthorizationCount > TcpResendNum)//dxl,2015.6.11,为兼容行标送检而修改
						{
							Link2LoginAuthorizationState = LOGIN_SEND_LOGIN_CMD;
						}
						else
