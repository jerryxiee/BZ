...
TIME_TASK MyTimerTask[MAX_TIMETASK] = {             
		...
		{ENABLE,0,5*SYSTICK_0p1SECOND,GpsMile_TimeTask},		        // 4:GPS里程计算,xx:xx:00开启  GpsMile_TimeTask,使能
		{ENABLE,0,1/*SYSTICK_1SECOND*/,Report_TimeTask},  	                // 5:位置汇报   Report_TimeTask,使能
        ...
        {ENABLE,0,SYSTICK_50ms*4,DoubtPoint_TimeTask},  	        // 13:疑点数据,使能,北斗功能检测时关闭,苗亚翰改为200ms调度1次    
        ...
        {ENABLE,0,1,Pulse_TimeTask},			// 23:检测脉冲速度,实际使用时xx:xx:00开启  ,使能,北斗功能检测时关闭
		{DISABLE,0,SYSTICK_1SECOND,Link1Login_TimeTask},		// 24:连接1登录鉴权   Link1Login_TimeTask
		...
		{ENABLE,1,SYSTICK_1SECOND,SpeedLog_TimeTask},               			// 31:速度异常记录,使能,北斗功能检测时关闭
		...
		{DISABLE,0,1,VDRCoefficient_TimeTask},                        // 42:脉冲自动校准  CoeffAdust_TimeTask,实际使用时才开启,V1.01.03版才开启
		...
		//{DISABLE,0,1,TimerUSART6HandleTask},                          // 46:可能影响上线功能因此关闭
        {ENABLE,0,1,Register_TimeTask},  		                      // 47:行驶记录（主要是变量监测和自检）
		{DISABLE,0,  1  ,ICCard_TimeTask},			// 48: IC卡各类事务,默认使用是开启的
#if(ICCARD_SEL==ICCARD_JTD)
		{DISABLE, 0, ICAUC_TIME_TICK, ICAUC_TimeTask},                   // 49:IC卡认证TCP连接任务,默认使用是开启的,第1个版本多中心连接不完善，因此关闭,dxl,2014.7.15
#else
        //{DISABLE, 0, ICAUC_TIME_TICK, ICAUC_TimeTask},                   //49:IC卡认证TCP连接任务,默认使用是开启的,第1个版本多中心连接不完善，因此关闭,dxl,2014.7.15
        {ENABLE, 0, 1, Card_TimeTask},                                     // 49:IC卡认证TCP连接任务
#endif
        ...
        {DISABLE,0,1*SECOND,Corner_TimeTask},                      //69:拐点补传,liamtsen 改为DISABLE
        {DISABLE,0,2,LZM_PublicTimerTask},                       //70:公共
        {DISABLE,0,SYSTICK_1SECOND,Link2Login_TimeTask},		// 71:连接2登录鉴权   Link2Login_TimeTask
        ...
        {DISABLE,0,1,TimerNopHandleTask},//75:陀螺仪接入
        {ENABLE,0,SECOND,DeepPlow_TimeTask},                      //76:耕深
        {ENABLE,0,SECOND,TurnSpeed_TimeTask},                      //77:转速
};
