...
#ifndef FRAM_CARLOAD_TOTAL_WEIGHT_ADDR////总重量
#define FRAM_CARLOAD_TOTAL_WEIGHT_ADDR      FRAM_15MIN_END_TIME_ADDR//总重量
#endif
...
typedef struct
{
    ...
    unsigned long totalWeight;      //上报重量 
    unsigned long preSaveWeight;   //上报重量 
    unsigned long showWeight;       //显示重量
    unsigned char clearCnt;         //传感器清零次数
    unsigned char CheckCnt;         //检测用，计数器
    unsigned char overloadflag;      //超载计数器
    unsigned char overloadCnt;      //超载计数器
    unsigned char normalloadCnt;    //未超载计数器
...
** 调用模块: 无
*************************************************************/
void CarLoad_ReadTotalWeight(void)
{
    unsigned long temp ;    
    if(0==Public_ReadDataFromFRAM(FRAM_CARLOAD_TOTAL_WEIGHT_ADDR,(unsigned char *)&temp,4))
    {
        temp = 0;       
    }
    /////////////////////
    s_stCarLoadAttrib.showWeight = temp; 
    s_stCarLoadAttrib.totalWeight = temp;
    s_stCarLoadAttrib.preSaveWeight = temp;
}
/*************************************************************
** 函数名称: CarLoad_SaveTotalWeight
** 功能描述: 
** 入口参数: 
** 出口参数: 无
** 返回参数: 无
...
** 调用模块: 无
*************************************************************/
void CarLoad_SaveTotalWeight(void)
{
    if((s_stCarLoadAttrib.totalWeight>s_stCarLoadAttrib.preSaveWeight+200)
     ||(s_stCarLoadAttrib.totalWeight+200<s_stCarLoadAttrib.preSaveWeight))
    {
        s_stCarLoadAttrib.preSaveWeight = s_stCarLoadAttrib.totalWeight;
        Public_WriteDataToFRAM(FRAM_CARLOAD_TOTAL_WEIGHT_ADDR,(unsigned char *)&s_stCarLoadAttrib.totalWeight,4);
    }
}
/*************************************************************
...
ProtocolACK CarLoad_PactAndSendData(unsigned char cmd,unsigned char CmdType,unsigned char *data,unsigned short datalen)
{
    ...
    ///////////////////////
    if((s_stCarLoadAttrib.contrecvtimeoutCnt>=5)&&(0==s_stCarLoadAttrib.EquipExistFlag))return ACK_ERROR;
    ////////////////////////////////
    ...
    if(1==s_stCarLoadAttrib.lcdTransFlag)s_stCarLoadAttrib.lcdTransFlag = 0;
    ///////////////////
...
void CarLoad_ResendData(void)
{
    if((s_stCarLoadAttrib.contrecvtimeoutCnt>=5)&&(0==s_stCarLoadAttrib.EquipExistFlag))
    {
        Io_WriteSelfDefine2Bit(DEFINE_BIT_26,SET);
        s_stCarLoadAttrib.CheckCnt = 0;
    ...
    if(result)
    {
        if(CARLOAD_COMM_CMD_QUERY_COM_WEIGHT == s_stCarLoadAttrib.sendCmd)
        {
            ...
            else
            {
                s_stCarLoadAttrib.onlinestatus = 0;
                Io_WriteSelfDefine2Bit(DEFINE_BIT_26,SET);
                if(0==s_stCarLoadAttrib.EquipExistFlag)
                return result;
            }
        }
    }
...
void Carload_SendCmdQuerySensorAbnormal(void)
{
    ...
    if(2==s_stCarLoadAttrib.lcdTransFlag)s_stCarLoadAttrib.lcdTransFlag = 0;
...
** 调用模块: 无
*************************************************************/
void Carload_DisposeSendCurCmdType(void)
{
    unsigned char data[1];
    LZM_PublicKillTimerAll(s_stCLTimer,CL_TIMERS_MAX);
    ///////////////
    if(1==s_stCarLoadAttrib.CurCmdType)
    {
        CarLoad_PactAndSendDataEx(CARLOAD_COMM_CMD_SENSOR_CLEAR,data,0); 
    }
    else
    if(2==s_stCarLoadAttrib.CurCmdType)
    {        
        s_stCarLoadAttrib.lcdTransFlag = 2;        
        CarLoad_PactAndSendDataEx(CARLOAD_COMM_CMD_QUERY_SENSOR_ABNORMAL,data,0);  
    }
    else
    if(3==s_stCarLoadAttrib.CurCmdType)
    {
        Carload_SendCmdQuerySensorAbnormal();
    }
    else
    if(4==s_stCarLoadAttrib.CurCmdType)
    {
        Carload_SendCmdClearServerSensorAbnormal();
    }
    else
    if(5==s_stCarLoadAttrib.CurCmdType)
    {
        CarLoad_SendData(s_stCarLoadTx.SendBuffer,s_stCarLoadTx.SendLen);
    }
    s_stCarLoadAttrib.CurCmdType = 0;
    LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_TASK],PUBLICSECS(3),Carload_SendCmdQueryTotalWeight);
}
/*************************************************************
** 函数名称: Carload_LcdSendCmdQuerySensorAbnormal
...
{
    if(LoadControllerUpdata_GetUpdataFlag())return;
    s_stCarLoadAttrib.CurCmdType = 2;
    LZM_PublicKillTimerAll(s_stCLTimer,CL_TIMERS_MAX);
    LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.4),Carload_DisposeSendCurCmdType);  
}
/*************************************************************
** 函数名称: Carload_SendCmdClearServerSensorAbnormal
** 功能描述: 清除传感器
...
{
    if(LoadControllerUpdata_GetUpdataFlag())return;
    s_stCarLoadAttrib.CurCmdType = 1;
    LZM_PublicKillTimerAll(s_stCLTimer,CL_TIMERS_MAX);    
    LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.4),Carload_DisposeSendCurCmdType);
}
/*************************************************************
...
{
    s_stCarLoadAttrib.overloadflag = 1;
...
{
    s_stCarLoadAttrib.overloadflag = 0;
...
{
    if(0xffffffff==s_stCarLoadAttrib.maxweight)return;
    if(!PUBLIC_CHECKBIT(s_stCarLoadAttrib.totalWeight,31))    
    {
        if(s_stCarLoadAttrib.totalWeight>=s_stCarLoadAttrib.maxweight)
        {
            s_stCarLoadAttrib.overloadCnt++;
            s_stCarLoadAttrib.normalloadCnt = 0;
        }
        else
        {
            s_stCarLoadAttrib.overloadCnt = 0;
            s_stCarLoadAttrib.normalloadCnt++;
        }        
    }
    else
    {
    ...
    //////////////////////////////
    if(s_stCarLoadAttrib.overloadCnt>=3)
    {
        s_stCarLoadAttrib.overloadCnt = 4;
        CarLoad_SetOverLoadAlarm();
    }
    else
    if(s_stCarLoadAttrib.normalloadCnt>=3)
    {
        s_stCarLoadAttrib.normalloadCnt = 4;
...
void CarLoad_DisposeRecvDataResponse(void)
{
    ...
    if(s_stCarLoadAttrib.CurCmdType)//
    {
        LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],LZM_AT_ONCE,Carload_DisposeSendCurCmdType);  
        return;
    }
    /////////////////
    ...
    switch(s_stCarLoadAttrib.recvCmd)
    {
        case CARLOAD_COMM_CMD_QUERY_COM_WEIGHT: 
            ...
            status = pBuffer[0];
            if(PUBLIC_CHECKBIT(status,1))////重量异常
            {
            ...
            {
                if(PUBLIC_CHECKBIT(status,2))//传感器异常
                {
                ...
                /////////////////
                s_stCarLoadAttrib.showWeight = Public_ConvertBufferToLong(&pBuffer[1]);
                ////////////////////////
                if(0==PUBLIC_CHECKBIT(status,0))//称量稳定
                {
                    s_stCarLoadAttrib.totalWeight = s_stCarLoadAttrib.showWeight;
                    CarLoad_SaveTotalWeight();
                }
        ...
        case CARLOAD_COMM_CMD_QUERY_SENSOR_ABNORMAL:
            ...
            //////////////////////////
            if(CARLOAD_RADIO_QUERY_SENSOR_ABNORMAL == s_stCarLoadAttrib.platformCmd)///
            {
                s_stCarLoadAttrib.platformCmd = CARLOAD_RADIO_CMD_MAX;
                LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.5),CarLoad_SendQuerySensorAbnormalResult);
                LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_TASK],PUBLICSECS(3),Carload_SendCmdQueryTotalWeight);
            }
            else
            {
                if(2!=s_stCarLoadAttrib.lcdTransFlag)
                LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_CAR_STATUS],PUBLICSECS(0.5),Carload_SendCmdCurCarStatus);
        ...
        case CARLOAD_COMM_CMD_SERVER_SENSOR_CLEAR:
            ...
            {
                s_stCarLoadAttrib.platformCmd = CARLOAD_RADIO_CMD_MAX;
                LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.5),CarLoad_SendClearSensorAbnormalResult);
                LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_TASK],PUBLICSECS(3),Carload_SendCmdQueryTotalWeight);
            }
            else
            {
...
unsigned char CarLoad_CommProtocolParse(unsigned char *ParseBuffer,unsigned short usDataLenght)
{
    ...
    s_stCarLoadAttrib.EquipExistFlag = 1;
    Io_WriteSelfDefine2Bit(DEFINE_BIT_26,RESET);
...
{
    return s_stCarLoadAttrib.showWeight;
}
/*************************************************************
...
{
    return Io_ReadSelfDefine2Bit(DEFINE_BIT_25);
}
/*************************************************************
...
void CarLoad_DisposeRadioProtocol(unsigned char cmd,unsigned char *pBuffer,unsigned short BufferLen)
{
    ...
    switch(cmd)
    {
        case CARLOAD_RADIO_QUERY_SENSOR_ABNORMAL:
            s_stCarLoadAttrib.CurCmdType = 3;
            s_stCarLoadAttrib.platformCmd = cmd;
            LZM_PublicKillTimerAll(s_stCLTimer,CL_TIMERS_MAX);    
            LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.4),Carload_DisposeSendCurCmdType);            
            break;
        case CARLOAD_RADIO_CLEAR_SENSOR_ABNORMAL:
            s_stCarLoadAttrib.CurCmdType = 4;
            s_stCarLoadAttrib.platformCmd = cmd;
            LZM_PublicKillTimerAll(s_stCLTimer,CL_TIMERS_MAX);    
            LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.4),Carload_DisposeSendCurCmdType);          
            break;
        case CARLOAD_RADIO_DEVELOP_PORT:
            if(BufferLen>CARLOAD_TX_BUFFER)break;
            s_stCarLoadAttrib.platformCmd = cmd;
            s_stCarLoadAttrib.CurCmdType = 5;
            LZM_PublicKillTimerAll(s_stCLTimer,CL_TIMERS_MAX); 
            LZM_PublicSetOnceTimer(&s_stCLTimer[CL_TIMER_SEND_DATA],PUBLICSECS(0.4),Carload_DisposeSendCurCmdType);
            s_stCarLoadTx.SendLen = BufferLen;
            memcpy(s_stCarLoadTx.SendBuffer,pBuffer,s_stCarLoadTx.SendLen);                    
