...
typedef enum 
{
    ...
    #ifdef	BB_INSPECT
    MENU_SYS_INSPECT,//add by zwb at 2017.9.14 //送检开关选择
    MENU_SYS_GPSPWRRST,
    #endif
    MENU_SYS_MAX
}eSTMENUSYS;
...
struct SUT_MENU s_sutMenuInspectKeySelect;
const char *s_apcMenuSystemInspectKeySelect[] = 
{
    "未签到检测",
    "部标1号机检测",
    "部标2号机检测",     //
...
void ShowProductInfo(unsigned char page)
{
    ...
    for(i=0; i<max; i++)
    {
        ...
        else
        {
            ...
            else if(9 == index)     //终端ID从存储读取   //修改时间2015.07.20 修改人:fanqinghai
            {
                    ...
                    else                    
                    {
                        if(4==s_stTaskAttrib.ControlMaxId)
                        {
                            ...
                            else
                            {
                                ...
                                s_stTaskAttrib.ControlId = 0;
                                s_stTaskAttrib.ControlMaxId =0;
																EepromPram_WritePram(E2_MAIN_SERVER_IP_ID, "jt1.gghypt.net", strlen("jt1.gghypt.net"));
																EepromPram_WritePram(E2_BACKUP_SERVER_IP_ID, "jt2.gghypt.net", strlen("jt2.gghypt.net"));
                                Communication_Open();
                                LZM_StartNewTask(TaskMenuSystemAdvanced);//
                            }
                        }
...
** 调用模块: 
*************************************************************/  
extern void Gnss_WarmBoot(void);
LZM_RET TaskGpsPwrRst(LZM_MESSAGE *Msg)
{
    switch(Msg->message)
    {
        case TASK_START:
            LcdClearScreen();
            LcdShowCaption("GPS热启动",0);
            LcdShowHorLine(0, 131, 18, 1);
            LcdShowCaption("是否热启动?",24);
            LcdShowHorLine(0, 131, 45, 1);
            LcdShowCaption("按<确认>热启动",47);
            return 1;
        case KEY_PRESS:            
            switch(TaskPara)
            {
...
** 调用模块: 
*************************************************************/  
LZM_RET TaskInspectKeySelect(LZM_MESSAGE *Msg)
{
		unsigned char val;
		...
		{
				case TASK_START:
						MenuInit(&s_sutMenuInspectKeySelect,s_apcMenuSystemInspectKeySelect);
						MenuShow(&s_sutMenuInspectKeySelect);	 
						s_stTaskAttrib.ControlId =0;
						return 1;
				case KEY_PRESS:
						if(s_stTaskAttrib.ControlId==0xff)break;
						...
						{ 							 
								case KEY_BACK:
										...
										break;
								case KEY_ENTER:
										val = MenuGetHandle(&s_sutMenuInspectKeySelect);
										*Buffer = 0x1 << val;
										if((DetectOption_machine1 <= val)&&(DetectOption_machine3 >= val))
										{ 
											*Buffer |= (0x1 << DetectOption_protocolTest);
											temp = 0xfff3fe58;
										}
										if(0 == EepromPram_WritePram(E2_UNLOGIN_TTS_PLAY_ID, Buffer, E2_UNLOGIN_TTS_PLAY_ID_LEN))
								    {
										...
										if(DetectOption_protocolTest == val)//794
										{
											temp = 0xfff3fe58;
								    }
										else if(DetectOption_functionTest == val)//808
										{
											temp = 0x9f0c665e;
								    }
										if(temp)
									    ...
									    if(0 == EepromPram_WritePram(E2_ALARM_MASK_ID, Buffer, E2_ALARM_MASK_LEN))
									    {
											...
											EepromPram_UpdateVariable(E2_ALARM_MASK_ID);
										}
										UpdateRunMode();
										break;
								default:
										MenuResponse(&s_sutMenuInspectKeySelect,TaskPara);
										break;
						}
						return 1;
				default:
                    ...
                    switch(MenuGetHandle(&s_sutMenuSystemAdvanced))
                    {
                        ...
                        case MENU_SYS_GYRO:
                           // LZM_StartNewTask(TaskMenuGyro);//dxl,2015.6.27,增加陀螺仪菜单
                            break;
												#ifdef	BB_INSPECT
												case MENU_SYS_INSPECT:
														LZM_StartNewTask(TaskInspectKeySelect);     //送检开关选择
														break;
												case MENU_SYS_GPSPWRRST:
														LZM_StartNewTask(TaskGpsPwrRst);     //送检开关选择
														break;
												#endif
            default:
                            break;                  
...
{ 
   ...
   float tmpVal;
    ...
    switch(Msg->message)
    {
        case TASK_START:
            LcdClearScreen();
            if(OilWear_Get16PINPortInsertBSJOilFlag())    //1接入，0未接入
              {
                  LcdShowStr(0, y_Location+1,"博实结  ", 0);
                  tmpVal=OilWear_GetCurOilVal();                  //博实结油量,0.1L/单位
            
                  sprintf(DisBuffer,"当前油量 %5.1fL ",tmpVal/10);
                  LcdShowStr(0, y_Location+17,DisBuffer, 0);
                    y_Location+=32;
            }     
            
            if(OilWear_Get16PINPortInsertJointechOilFlag())  //1接入，0未接入
            {
...
LZM_RET TaskQueryPeripheralInform(LZM_MESSAGE *Msg)
{
    const char *Dynamic_pointer[6]=NULL;
