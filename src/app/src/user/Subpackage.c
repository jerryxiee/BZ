...
FunctionalState Subpackage_UploadTimeTask(void)
{
  ...
  if( 1 == SubpackageList[i] )//第1包
  {
      ...
      *p++ = 0;
      BufferLen += 1;
       //数据块，获取包数据
      length = Register_GetPacketData( p , SubpacketUploadType ,  SubpackageList[i]  );
      *q++ = ( length&0xff00 ) >> 8;
      *q++ = length&0xff;
      BufferLen += length ;
      //校验字节
  ...
  else//其它的分包补传还没有做
  {
      ...
      { 
          extern    u8 subSerialFlag;//抽检用，liamtsen
          subSerialFlag=1;
...
u8 Subpackage_Media_File_ID_Inc( void  )
{
  ...
  {
    //liamtsen  //g_sttSubPacketTransterCtrl.PacketCntCursor = g_sttSubPacketTransterCtrl.PacketIDArray[ g_sttSubPacketTransterCtrl.PacketCntCursor ]-1 ; 
      g_sttSubPacketTransterCtrl.PacketCntCursor = g_sttSubPacketTransterCtrl.PacketIDArray[ g_sttSubPacketTransterCtrl.PacketIDCntCursor ] ; 
      ret = 1 ;
      return ret ;
  }
...
//        if(InPackIndex==1)
//        {
//              len = ( u16 ) Subpackage_Media_File_Buf_Data_Get( pInfile , InPackIndex , pOutBuf);
//        }
//        else
//        {
              len = ( u16 ) Subpackage_Media_File_Buf_Data_Get( pInfile , InPackIndex-1 , pOutBuf);
//        }
  }
  else
  {
