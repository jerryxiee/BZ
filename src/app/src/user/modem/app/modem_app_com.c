...
/******************************************************************************///EGS701
//#ifdef   EYE_EGS701,dxl,2015.6.11,清除变量BlindSaveEnableFlag所出现的所有地方，不再使用了。
    //u8 BlindSaveEnableFlag = 0;//盲区记录使能标志,1为使能
/******************************************************************************///EGS702
//#elif    defined EYE_EGS702
    //extern u8 BlindSaveEnableFlag;
/******************************************************************************///缺省
//#else
    //u8 BlindSaveEnableFlag = 0;
///#endif
...
********************************************************************************
*/
extern u8   Link1ManualCloseFlag;//连接1手动关闭标志,1为关闭,0为开启
extern u8   Link2ManualCloseFlag;//连接2手动关闭标志,1为关闭,0为开启
/*
********************************************************************************
*                           FUNCTIONS EXTERN
********************************************************************************
...
FunctionalState Communication_CloseGprsTimeTask(void)
{
  /* dxl,2015.6.11屏蔽
      ...
      if(count < 10)//modify by joneming 20130624
      {
              MOD_CloseMainIp();//模块主动断开连接
      }
      else if(10 == count)
      {
              Communication_Close2();
      }
      else if(11 == count)
      {
              count = 0;
...
#ifdef EYE_MODEM
    Net_First_Open();
    //BlindSaveEnableFlag = 0;dxl,2015.6.11,清除变量BlindSaveEnableFlag所出现的所有地方，不再使用了。
#endif   
  */
...
#ifdef EYE_MODEM
    Net_First_Close();  
    //BlindSaveEnableFlag = 1;dxl,2015.6.11,清除变量BlindSaveEnableFlag所出现的所有地方，不再使用了。
#endif  
*/  
    if(0 == Link1ManualCloseFlag)
    {
        Net_First_Close();
        ...
        //{ 
	      //  ClearTerminalAuthorizationCode(CHANNEL_DATA_1);//dxl,2015.6.11
				//}
    }
}
/*******************************************************************************
...
void communication_CalTimer(void)
{
