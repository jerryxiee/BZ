...
{
    Register_Write(REGISTER_TYPE_OVER_TIME,pBuffer,length,endtime);
}
/*************************************************************
...
void TiredDrive_CheckDriverRunStatus(unsigned long currentTime,unsigned char index)
{
    ...
    if(1 == s_stTiredDriver.driverinfo[index].validflag)
    {
        ...
        if(1 ==RunStatus)//行驶中
        {
            ...
            ////////////////
            sucRunStatusCnt++;
            if(s_stTiredAttrib.prespeedVal==0&&curSpeedVal&&(curSpeedVal<SpeedMonitor_GetSpeedMaxLimitVal()))////速度为零到不为零
            {
                  Report_UploadPositionInfo(CHANNEL_DATA_1);//liamtsen  2017-09-19 add
                  Report_UploadPositionInfo(CHANNEL_DATA_2);//liamtsen  2017-09-19 add
            }
            //更新FRAM中的停车时间及停车位置,
    ...
    if(1 ==RunStatus)//行驶中,首次进入计时有效
    {
        ...
        if(s_stTiredDriver.driverinfo[index].starttime>currentTime)
        {
            TiredDrive_SaveCurStartInfo(index,currentTime);//实际开车时间 
        }
        else        
        {
            unsigned short time;
            time = s_stTiredAttrib.runDuration;
            if(time<10)time=10;
            if(s_stTiredDriver.driverinfo[index].starttime+time<currentTime)//
            {
                TiredDrive_SaveCurStartInfo(index,currentTime);//实际开车时间 
            }
        }
    ...
    else
    {
        ...
        {
            TiredDrive_SaveCurStartInfo(index,currentTime);//实际开车时间
            if(curSpeedVal<SpeedMonitor_GetSpeedMaxLimitVal())
            	{
            Report_UploadPositionInfo(CHANNEL_DATA_1);//liamtsen  2017-09-19 add
            Report_UploadPositionInfo(CHANNEL_DATA_2);//liamtsen  2017-09-19 add  
            	}
...
void TiredDrive_DisposeOneDriverInfoTimeIsAbnormal(unsigned char index)
{
    ...
    //////////////////
    if(0==s_stTiredDriver.driverinfo[index].validflag&&0==s_stTiredDriver.driverinfo[index].starttime)return;
    /////////////////
    currentTime = RTC_GetCounter();
    /////////////////////////
    if(currentTime>=s_stTiredAttrib.curTimeVal)//当前时间比之前时间晚
    {
        temp = currentTime - s_stTiredAttrib.curTimeVal;
        s_stTiredDriver.driverinfo[index].starttime += temp;
        s_stTiredDriver.driverinfo[index].endtime   += temp;
    }
    else//当前时间比之前时间早
    {
        temp = s_stTiredAttrib.curTimeVal-currentTime;
        if(s_stTiredDriver.driverinfo[index].starttime<temp)
        {
            temp = s_stTiredDriver.driverinfo[index].starttime;
            if(temp)temp--;
        }
        s_stTiredDriver.driverinfo[index].starttime -= temp;
        s_stTiredDriver.driverinfo[index].endtime   -= temp;
        if(s_stTiredDriver.driverinfo[index].endtime > currentTime)//时间错乱
        {
            s_stTiredDriver.driverinfo[index].validflag =0;
            TiredDrive_ResetCurDriverData(index);
            return;
...
unsigned short TiredDrive_ReadTiredRecord(unsigned char *pBuffer, TIME_T StartTime, TIME_T EndTime,u16 MaxBlock)
{
    ...
    /////////////////
    len += Register_Read(REGISTER_TYPE_OVER_TIME,p,StartTime,EndTime,MaxBlock-block);
    return len;    
}
/*************************************************************
...
unsigned short TiredDrive_Read2daysTiredRecord(unsigned char *pBuffer, TIME_T StartTime)
{
    ...
    //查找超时驾驶记录
    length = TiredDrive_ReadTiredRecord(pBuffer, StartTime,EndTime,12);
...
** 调用模块: 无
*************************************************************/
void TiredDrive_CheckAndDisposeStartTimeAbnormal(void) 
{
    unsigned char index;
    index = s_stTiredDriver.currentindex;
    if(s_stTiredDriver.driverinfo[index].validflag)
    {
        if(s_stTiredDriver.driverinfo[index].endtime> RTC_GetCounter())
        {
            s_stTiredAttrib.curTimeVal = s_stTiredDriver.driverinfo[index].endtime;
            TiredDrive_DisposeCurrentTimeIsAbnormal();
            s_stTiredAttrib.needsaveflag = 1;//需要保存
            s_stTiredAttrib.timeAbnormalFlag = 1;
        }
...
void TiredDrive_ParamInitialize(void) 
{
    ...
    ////////////////////////
    TiredDrive_CheckAndDisposeStartTimeAbnormal();
    ////////////////////////////
...
***************************************************************************/
void TiredDrive_ClearStartTime(void)
{
  s_stTiredDriver.driverinfo[s_stTiredDriver.currentindex].starttime = 0;
  s_stTiredDriver.driverinfo[s_stTiredDriver.currentindex].endtime = 0;
}
