...
/**
  ...
  * @retval None
  */
void Vdr_Time_Get(const u8 *pSrc, VDR_SERT *pTime,u8 cmd)
{
    //检查数据合法性
    ...
    /////////////
    pTime->cnt_start = ConverseGmtime(&pTime->t_start);
    //////////////////
    pTime->cnt_end = ConverseGmtime(&pTime->t_end)+1;
    /////以下是串口查询使用到////////////
    if(0x08 == cmd||0x09 == cmd)//速度记录//位置记录
    {
        Gmtime(&pTime->t_end,pTime->cnt_end);
    }    
}
...
void Vdr_Wire_Even(VDR_CH ch, VDR_HEAD head)
{
    ...
    {
        Vdr_Time_Get(head.pbuf,&serch_time,head.cmd);
...
/**
  ...
  *         错误返回 非0
  */
void Vdr_Data_Send(VDR_CH ch, VDR_HEAD headsend)
{
    MESSAGE_HEAD msg_head;
    u16 len;
    u16 addr;
    ...
    if(ch == VDR_COM)
    {
        len = Vdr_Head_ToBuf(0,headsend,Vdr_BufSend);
    ...
    else
    {
        ...
        Vdr_BufSend[addr++] = headsend.cmd;
        len = Vdr_Head_ToBuf(0,headsend,Vdr_BufSend+addr);//转为字节流
        len += addr;    
...
/**
  ...
  * @retval 返回处理结果
  */
u8 Vdr_Data_Pro(VDR_CH ch, VDR_HEAD headrev, VDR_HEAD *pHeadsend)
{
    VDR_SERT  serch_time;
    u8  addr;
    u8  type;
    ...
    ////////////////////////////////////////////////////////////////////转换命令
    if(Vdr_Cmd_Cnv(headrev.cmd,&type,&addr) ==0 )
    {
        return 0;//非法数据
    }
    ...
    switch(type)
    {
        ...
        case 1://采集指令
        {
            if(ch == VDR_COM)//串口
            {
                Vdr_Time_Get(headrev.pbuf,&serch_time,headrev.cmd);
                Vdr_Get_Ser(addr,serch_time,pHeadsend);
            }
            else//无线采集
            {   
                Vdr_Wire_Even(ch,headrev);
                return 0;
            }
            break;
        }
        case 2://设置指令
        {
            Vdr_Set_Pro(addr,headrev.pbuf,pHeadsend);
            break;
        }
        case 3://鉴定指令
        {
            Vdr_Test_Pro(addr,pHeadsend);
            break;
        } 
        case 4://伊爱指令
        {
            Vdr_Eye_Value(&headrev,pHeadsend);
            break;
        }        
    }
    return 1;
