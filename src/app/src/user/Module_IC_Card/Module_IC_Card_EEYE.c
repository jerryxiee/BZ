...
/////////////////
#define ICCARD_DRIVER_SIGN_DATA_LEN_MAX        99//
typedef struct
{
...
void ICCard_SaveRegisterTypeDriver(unsigned char status,unsigned long time)
{
    ...
    Register_Write( REGISTER_TYPE_DRIVER, buffer, (REGISTER_DRIVER_STEP_LEN-5),time);
}
/*************************************************************
...
{
    if(DRIVER_SIGN_IN!=status&&DRIVER_SIGN_OUT!=status)return;
...
** 调用模块: 
*************************************************************/
unsigned char ICCard_PackReportDriverInfo(unsigned char *pBuffer)
{
    if(s_stDriverSign.len)
    {
        memcpy(pBuffer,s_stDriverSign.data,s_stDriverSign.len);
    }
    return s_stDriverSign.len;
}
...
void ICCard_ProtocolParse(unsigned char * pInBuf, unsigned short InLength)
{
    ...
    unsigned short iPosition[2],datalen;
    unsigned short i;
    unsigned char *buffer;
    unsigned char flag;
...
void ICCard_DisposeCommTimeOut(void)
{
    ...
    s_stICCardAttrib.onlinestatus = ICCARD_ONLINE_STASUS_OFF;
		if(DetectStatus(DetectOption_functionTest))return;
...
{
	if( s_stICCardAttrib.signstatus != DRIVER_SIGN_IN) return 0;
	memcpy(pBuffer,&s_stICCardAttrib.stDriver.DriverID,18);
	return 18;
}
/*************************************************************
