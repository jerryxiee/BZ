...
static u8 Link1BlindSaveEnableFlag = 0;//liamtsen

/********************全局变量*************************/
static u16 Link2OpenFlag = 0;//连接2是否开启标志，1为开启，0为未开启
static s16  BlindStepLen = 0;//盲区每步的长度,liamtsen
static s16  BlindPacketNum = 0;//盲区补报时打包的条数，实际出货时为5条，检测时为15-20条,liamtsen
...
extern void LcdShowCaptionEx(char *str,unsigned char y);//liamtsen 2017-09-13
extern void LcdClearScreen(void);//liamtsen 2017-09-13
/********************函数定义*************************/
/**************************************************************************
//函数名：Blind_TimeTask
//功能：实现盲区补报功能
...
FunctionalState Blind_TimeTask(void)
{
	...
	if((DetectStatus(DetectOption_functionTest))&&(Link1BlindSaveEnableFlag==1))
		{
			return ENABLE;
		}//liamtsen
	/**************连接1盲区补报****************/
...
u8 Blind_Save(u8 channel, u8 *pBuffer, u8 length)
{
	...
	if((DetectStatus(DetectOption_functionTest))&&(0 == Blind_GetLink1SaveFlag()))
	{
		return flag;
	}
	attribute = channel;
...
//备注：
***************************************************************************/
u8 Blind_GetLink1SaveFlag(void)
{
    return Link1BlindSaveEnableFlag;
}
...
static void Blind_Link1Init(void)
{
	...
	UpdateRunMode();//liamtsen
	if( DetectStatus(DetectOption_functionTest))//dxl,2016.4.7兼容部标检测和实际应用
	{
	...
	Link1Blind.SectorStep = FLASH_ONE_SECTOR_BYTES/BlindStepLen;//liamtsen
	//Link1Blind.SectorStep = FLASH_ONE_SECTOR_BYTES/FLASH_BLIND_STEP_LEN;
	...
	BlindPacketNum = 800/BlindStepLen;//liamtsen 2017-09-13,盲区补报打包条数改成可变


	//for(i=0; i<5; i++)//liamtsen 2017-09-13
...
static void Blind_Link1SearchSector(s16 SearchSector)
{
	...
	Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + Link1Blind.SectorStep*BlindStepLen + 1;//liamtsen 2017-09-13
	...
	if(0xaa == flag)
	{
		...
		Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + (Link1Blind.SectorStep-1)*BlindStepLen;//liamtsen 2017-09-13
	...
	Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + (Link1Blind.SectorStep-1)*BlindStepLen;//读扇区的最后1步,liamtsen 2017-09-13
	sFLASH_ReadBuffer(Buffer,Addr,4);
	ttCount = Public_ConvertBufferToLong(Buffer);
	if(0xffffffff != ttCount)
	{
		...
		if(SUCCESS == CheckTimeStruct(&tt))
		{
			...
			sFLASH_ReadBuffer(Buffer,Addr,BlindStepLen);//liamtsen 2017-09-13
			...
			else
			{
				...
				if((ttCount > Link1Blind.OldestTimeCount)&&(0xaa == Buffer[BlindStepLen-1]))//liamtsen 2017-09-13
				{
					Link1Blind.OldestSector = SearchSector;
					Link1Blind.OldestStep = 0;
					Link1Blind.OldestTimeCount = ttCount;
	...
	else
	{
		...
		if(0xffffffff != ttCount)
		{
			for(i=0; i<Link1Blind.SectorStep; i++)
			{
				...
				Addr = SearchSector*FLASH_ONE_SECTOR_BYTES+i*BlindStepLen;//liamtsen 2017-09-13
				//sFLASH_ReadBuffer(Buffer,Addr,FLASH_BLIND_STEP_LEN);
				sFLASH_ReadBuffer(Buffer,Addr,BlindStepLen);//liamtsen 2017-09-13
				ttCount = Public_ConvertBufferToLong(Buffer);
				if(0xffffffff == ttCount)
...
static u8 Blind_Link1Report(void)
{
	...
	for(count=0; count<BlindPacketNum; )//每包包含5条位置信息 liamtsen 2017-09-13
	{
		...
		Addr = Link1Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + Link1Blind.SectorStep*BlindStepLen+ 1;//liamtsen 2017-09-13
		sFLASH_ReadBuffer(&flag,Addr,1);//读整个扇区是否已补报标志
		if(0xaa == flag)//已补报过
		{
		...
		{
			//for(;count<5;)
			for(;count<BlindPacketNum;)//liamtsen 2017-09-13
			{
				if(0 == Blind_GetLink1ReportFlag())
				{
					break;
				...
				Addr = Link1Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + (Link1Blind.ReportStep+1)*BlindStepLen- 1;//liamtsen 2017-09-13
				...
				else
				{
					...
					Addr = Link1Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + Link1Blind.ReportStep*BlindStepLen;//liamtsen 2017-09-13
					//sFLASH_ReadBuffer(Buffer,Addr,FLASH_BLIND_STEP_LEN);
					sFLASH_ReadBuffer(Buffer,Addr,BlindStepLen);//liamtsen 2017-09-13
					BufferLen = Buffer[6];
					sum = 0;
					...
					if(Buffer[5] == sum)
					{
						//if(((length+BufferLen+2) < RADIO_PROTOCOL_BUFFER_SIZE)&&(BufferLen < (FLASH_BLIND_STEP_LEN-7)))
						if(((length+BufferLen+2) < RADIO_PROTOCOL_BUFFER_SIZE)&&(BufferLen < (BlindStepLen-7)))//liamtsen 2017-09-13
						{
...
static void Blind_Link1ReportAck(void)
{
	...
	//for(i=0; i<5; i++)
	for(i=0; i<BlindPacketNum; i++)//liamtsen 2017-09-13
	{
				...
				&&(Link1Blind.LastReportStep[i] < Link1Blind.SectorStep))
		{
			...
			Addr = Link1Blind.LastReportSector[i]*FLASH_ONE_SECTOR_BYTES+(Link1Blind.LastReportStep[i]+1)*BlindStepLen- 1;//liamtsen 2017-09-13
			flag = 0xaa;
			...
			if(Link1Blind.LastReportStep[i] >= (Link1Blind.SectorStep-1))
			{
				...
				Addr = Sector*FLASH_ONE_SECTOR_BYTES+Link1Blind.SectorStep*BlindStepLen + 1;//liamtsen 2017-09-13
				flag = 0xaa;
...
static u8 Blind_Link1Save(u8 *pBuffer, u8 length, u8 attribute)
{
	...
	TIME_T tt;
	u32 ttCount;
	if(DetectStatus(DetectOption_functionTest)&&(0==Link1BlindSaveEnableFlag))
		{
		return 1;
		}//liamtsen
	...
	else//不为空，已存有数据
	{
		...
		Addr = Sector*FLASH_ONE_SECTOR_BYTES+Step*BlindStepLen;//liamtsen 2017-09-13
		sFLASH_ReadBuffer(Buffer,Addr,4);
		ttCount = Public_ConvertBufferToLong(Buffer);
		Gmtime(&tt,ttCount);
	...
	Addr = Link1Blind.SaveSector*FLASH_ONE_SECTOR_BYTES+Link1Blind.SaveStep*BlindStepLen;//检查当前存储的那一步的地址是否为空,liamtsen 2017-09-13
...
static void Blind_Link2Init(void)
{
	...
	Link2Blind.SectorStep = FLASH_ONE_SECTOR_BYTES/BlindStepLen;//liamtsen 2017-09-13
...
static void Blind_Link2SearchSector(s16 SearchSector)
{
	...
	Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + Link2Blind.SectorStep*BlindStepLen+ 1;//liamtsen 2017-09-13
	sFLASH_ReadBuffer(&flag,Addr,1);//读整个扇区是否已补报标志
	if(0xaa == flag)
	{
		//Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + (Link2Blind.SectorStep-1)*FLASH_BLIND_STEP_LEN;
		Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + (Link2Blind.SectorStep-1)*BlindStepLen;//liamtsen 2017-09-13
		sFLASH_ReadBuffer(Buffer,Addr,4);
		ttCount = Public_ConvertBufferToLong(Buffer);
		Gmtime(&tt,ttCount);
		if(SUCCESS == CheckTimeStruct(&tt))
	...
	Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + (Link2Blind.SectorStep-1)*BlindStepLen;//读扇区最后1步 liamtsen 2017-09-13
	sFLASH_ReadBuffer(Buffer,Addr,4);
	ttCount = Public_ConvertBufferToLong(Buffer);
	if(0xffffffff != ttCount)
	{
		...
		if(SUCCESS == CheckTimeStruct(&tt))
		{
			...
			else
			{
				...
				if((ttCount > Link2Blind.OldestTimeCount)&&(0xaa == Buffer[BlindStepLen-1]))//liamtsen 2017-09-13
				{
					Link2Blind.OldestSector = SearchSector;
					Link2Blind.OldestStep = 0;
					Link2Blind.OldestTimeCount = ttCount;
	...
	else
	{
		...
		if(0xffffffff != ttCount)
		{
			for(i=0; i<Link2Blind.SectorStep; i++)
			{
				//Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + i*FLASH_BLIND_STEP_LEN;
				//sFLASH_ReadBuffer(Buffer,Addr,FLASH_BLIND_STEP_LEN);
				Addr = SearchSector*FLASH_ONE_SECTOR_BYTES + i*BlindStepLen;//liamtsne 2017-09-13
				sFLASH_ReadBuffer(Buffer,Addr,BlindStepLen);//liamtsne 2017-09-13
				ttCount = Public_ConvertBufferToLong(Buffer);
				if(0xffffffff != ttCount)
				{
					Gmtime(&tt,ttCount);
...
static u8 Blind_Link2Report(void)
{
	...
	for(count=0; count<BlindPacketNum; )//每包包含5条位置信息,liamtsen 2017-09-13
	{
		...
		//Addr = Link2Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + Link2Blind.SectorStep*FLASH_BLIND_STEP_LEN + 1;
         Addr = Link2Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + Link2Blind.SectorStep*BlindStepLen + 1;//liamtsen 2017-09-13
		sFLASH_ReadBuffer(&flag,Addr,1);//读整个扇区是否已补报标志
		if(0xaa == flag)//已补报过
		{
		...
		else
		{
			...
			for(;count<BlindPacketNum;)//liamtsen 2017-09-13
			{
				...
				Addr = Link2Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + (Link2Blind.ReportStep+1)*BlindStepLen- 1;//liamtsen 2017-09-13
				sFLASH_ReadBuffer(&flag,Addr,1);//读该条是否已补报标志
				if(0xaa == flag)//已补报过
				{
				...
				else
				{
					...
					Addr = Link2Blind.ReportSector*FLASH_ONE_SECTOR_BYTES + Link2Blind.ReportStep*BlindStepLen;//liamtsen 2017-09-13
					//sFLASH_ReadBuffer(Buffer,Addr,FLASH_BLIND_STEP_LEN);
					sFLASH_ReadBuffer(Buffer,Addr,BlindStepLen);//liamtsen 2017-09-13
					BufferLen = Buffer[6];
					sum = 0;
	...
	{
		//for(i=count; i<5; i++)
		for(i=count; i<BlindPacketNum; i++)//liamtsen 2017-09-13
		{
			Link2Blind.LastReportSector[i] = 0;
			Link2Blind.LastReportStep[i] = 0;
		}
...
static void Blind_Link2ReportAck(void)
{
	...
	for(i=0; i<BlindPacketNum; i++)//liamtsen 2017-09-13
	{
				...
				&&(Link2Blind.LastReportStep[i] < Link2Blind.SectorStep))
		{
			...
			Addr = Link2Blind.LastReportSector[i]*FLASH_ONE_SECTOR_BYTES+(Link2Blind.LastReportStep[i]+1)*BlindStepLen- 1;//liamtsen 2017-09-13
			flag = 0xaa;
			sFLASH_WriteBuffer(&flag, Addr, 1);//标记该条盲区已上报
			...
			if(Link2Blind.LastReportStep[i] >= (Link2Blind.SectorStep-1))
			{
				...
				Addr = Sector*FLASH_ONE_SECTOR_BYTES+Link2Blind.SectorStep*BlindStepLen + 1;//liamtsen 2017-09-13
				flag = 0xaa;
				sFLASH_WriteBuffer(&flag, Addr, 1);//标记该扇区的盲区已上报
			}
		}
...
static u8 Blind_Link2Save(u8 *pBuffer, u8 length, u8 attribute)
{
	...
	if((length > (BlindStepLen-9))||(length < 28)||(0 == Link2Blind.SaveEnableFlag))//每步最后两个字节用作错误标记，已上传标记,liamtsen 2017-09-13
	{
		return 1;
	}
	...
	Addr = Link2Blind.SaveSector*FLASH_ONE_SECTOR_BYTES+Link2Blind.SaveStep*BlindStepLen;//检查当前存储的那一步的地址是否为空 liamtsen 2017-09-13
	sFLASH_ReadBuffer(Buffer,Addr,5);
	for(i=0; i<5; i++)
	{
		if(0xff != Buffer[i])
