...
void CAN_Isr_Rx( CAN_TypeDef* CANx )
{
  ...
  if(clearflag==0)
  {
  gCANParame[iCANCh].UploadTimeCnt=0;//清除采集超时时间,防止超时时间到还没有收到数据导致第一包can数据总包数不是50包
  clearflag=1;
  } 
  //liamtsen
  ...
  //  memcpy((u8*)&gCanIsrData[iCANCh].receBuffer[num].CAN_ID_BYTE_0, (u8*)(&RxMessage.ExtId), 4);
	...
	{
	 gCanIsrData[iCANCh].receBuffer[num].CAN_Frame_Type = 0; //bit 30: 表示帧类型: 0:标准帧 1:扩展帧
	}//liamtsen 2017-09-14检测平台有bug,是扩展帧时不能将bit30置位
  }
            ...
            if( ! Flag_First_Valid_CANPacket){
                  ...
                  if(DetectStatus(DetectOption_functionTest))
                  {
                        if(firstFlag==0)
            		{
                              ...
                              RTC_GetCurTime(&Time);
                              Time.hour = Public_BCD2HEX(gCanIsrData[0].receBuffer[readindex].CAN_Time[0]);
                              Time.min =  Public_BCD2HEX(gCanIsrData[0].receBuffer[readindex].CAN_Time[1]);
                              Time.sec =  Public_BCD2HEX(gCanIsrData[0].receBuffer[readindex].CAN_Time[2]);
                        ...
                        else
                        {
                              ...
                              *dataAddr++ =	Public_HEX2BCD(Time.hour);
                              *dataAddr++ =	Public_HEX2BCD(Time.min);
                              *dataAddr++ =	Public_HEX2BCD(Time.sec);
                              *dataAddr++ =	gCanIsrData[0].receBuffer[readindex].CAN_Time[3] ; 
                              *dataAddr++ =	gCanIsrData[0].receBuffer[readindex].CAN_Time[4] ;
                              retlenght +=5;
