...
unsigned char E2ParamApp_ReadSpecialParam(unsigned short usParamID,unsigned char *data)
{
    ...
    {
        Public_ReadDataFromFRAM(FRAM_MILEAGE_ADDR,buffer,FRAM_MILEAGE_LEN);//单位0.01公里
        val  = Public_ConvertBufferToLong(buffer);
        val *= 0.1;
...
** 调用模块: 
*************************************************************/	
void E2ParamApp_ResetDefaults(void)
{
		BEEP_OFF();
    //擦除盲区记录
    Blind_Erase(0xff);
		if(0 == DetectMultiStatus(0,2,DetectOption_functionTest,DetectOption_protocolTest))//部标功能检测,部标协议检测
    {
	    //恢复为第1次使用时自动配置的参数
        ...
        if(ID&&ID< E2_PRAM_ID_INDEX_MAX)//处理ID已识别的,其它跳过
        {
            ...
            if(EepromPramLength[ID] >= PramLen)
            {
                ...
                if(0 == flag)
                {
                    ...
                    {
						if(DetectMultiStatus(0,2,DetectOption_functionTest,DetectOption_protocolTest))//liamtsen 2017-09-13
							{
							}
						else
							{
					...
					if(E2_BACKUP_SERVER_IP_ID == PramID)//dxl,2016.5.13把设置备份服务器作为多中心连接功能的开启标志
						{
							...
							else if(DetectStatus(DetectOption_protocolTest))
								{
