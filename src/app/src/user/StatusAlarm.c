...
FunctionalState StatusAlarm_TimeTask(void)
{
  ...
  u8	flag;      
	...
	if(1 == EmergencyFlag)
	{
		...
		else if((200 == EmergencyDelayCount)&&(DetectStatus(DetectOption_protocolTest)))//10秒后清除，协议检测要求紧急报警自动清除
		{
	...
	if((1 == PhotoEnable)&&(EnterCount >= 60*SECOND))//上电60秒以后才允许拍照
	{
        ...
        //状态变化立即上报
        if(((LastStatus != Status)&&(0 == ReportFlag))&&(EnterCount >= 30*SECOND))//上电30秒以后才允许
        {
                Report_UploadPositionInfo(CHANNEL_DATA_1|CHANNEL_DATA_2);//上报一条位置信息
                ReportFlag = 1;
        }
        //产生报警立即上报
        if(((LastAlarm != Alarm)&&(0 == ReportFlag))&&(EnterCount >= 30*SECOND))//上电30秒以后才允许
        {
              if( DetectStatus(DetectOption_functionTest))//liaamtsen 2017-09-18报警发生时才立即上报，清除时没有要求；立即上报可能反而带来麻烦，导致检测不通过
              {
                    temp = LastAlarm^Alarm; 
                    if(temp == (Alarm&temp))
                    {
