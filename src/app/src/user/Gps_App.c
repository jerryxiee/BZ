...
void Gps_IsInRunning(void)
{
    ...
    ////////////////////
    if(Task_GetCurSystemIsDeepFlag())//耕深//add by joneming
    {
        checkSpeed  = 1;//add by joneming
    }
    /////////////////
    Speed = (Position.Speed*18520+Position.SpeedX*1852+5000)/10000;//add by joneming,把精度提高0.5km
    ///////////////////////
    if(Speed > checkSpeed)//dxl,2014.8.26,原来是速度大于0,现在纠正为5km/h
    {
        DownCount = 0;
        UpCount++;
        if(UpCount >= 10)
...
void Gps_EvTask(void)
{
	...
	if((DetectMultiStatus(0,3,DetectOption_functionTest,DetectOption_protocolTest,DetectOption_gbTest))&&(0 == ACC))
	{
	...
	if(1 == VerifyFlag)//通过
	{
		...
		if(DetectStatus(DetectOption_machine3))//协议检测3号机测试时（脉冲速度模式）过滤GPS数据
		{
		    flag = 0;
		}
		if(1 != flag)
		{
			...
			if(DetectStatus(DetectOption_gbTest))//国标检测时不能校时
			{
...
FunctionalState  Gps_TimeTask(void)
{
    ...
    Gps_IsInRunning();
		...
		if(DetectMultiStatus(0,3,DetectOption_functionTest,DetectOption_protocolTest,DetectOption_gbTest))
		{
			   return ENABLE;
	  }
