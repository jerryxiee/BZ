...
typedef struct
{
    ...
    unsigned char checkTimeCnt;              //当前时间
...
unsigned long SpeedMonitor_GetLastStopTime(void)
{
    ...
    TIME_T stTime;
    if(Public_ReadDataFromFRAM(FRAM_STOP_15MIN_TIME_ADDR,buffer,6))
    {
...
void SpeedMonitor_ReadSpeedPlay(void) 
{
    ...
    {
        timeVal = s_stSpeedAttrib.curTimeVal;
...
void SpeedMonitor_SaveRecordDataToFlash(void)
{
    ...
    Register_Write(REGISTER_TYPE_SPEED,buffer,ONE_SPEED_MONITOR_DATA_LEN,s_stSpSaveRecord.startTime);
    SpeedMonitor_ClearRunRecordData();
}
/*************************************************************
...
void SpeedMonitor_CheckRecordDataToSaveFlash(void)
{
    ...
    curTime = s_stSpeedAttrib.curTimeVal;
...
/*******************************************************************************
* Function Name  : SpeedMonitor_SaveStop15minTimeToFram
* Description    : 保存15分钟的停车时间
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void SpeedMonitor_SaveStop15minTimeToFram(void)
{
    unsigned char buffer[10];
    unsigned long timecnt;
    TIME_T time;
    timecnt =RTC_GetCounter();
    Gmtime(&time,timecnt-RUN_CHAGE_STATUS_TIME);
    Public_ConvertTimeToBCDEx(time,buffer);            
    FRAM_BufferWrite(FRAM_STOP_15MIN_TIME_ADDR,buffer,6);//刷新停车时间
}
/*************************************************************
** 函数名称: SpeedMonitor_SaveRunRecordData
** 功能描述: 保存行驶记录(以1s的时间间隔持续记录并存储车辆行驶状态数据。该行驶状态数据为：车辆在行驶过程中的实时时间、每秒钟间隔内对应的平均速度以及对应时间的状态信号)
...
void SpeedMonitor_SaveRunRecordData(unsigned char CurrentSpeed)
{
    ...
    if((CAR_STATUS_STOP==s_stSpeedAttrib.prerunStatus)&&(CAR_STATUS_RUN==s_stSpeedAttrib.runStatus))//由停驶到行驶
    {
        ...
        {
            SpeedMonitor_SavePackRecordDataToFRAM(s_stSpeedAttrib.recordCnt,s_stSpeedAttrib.curTimeVal);
        }
    ...
    if((CAR_STATUS_RUN==s_stSpeedAttrib.prerunStatus)&&(CAR_STATUS_STOP==s_stSpeedAttrib.runStatus))//由行驶到停驶
    {
        ...
        s_stSpeedAttrib.recordCnt = 0;
        ////////保存停车时间/////////////
        SpeedMonitor_SaveStop15minTimeToFram();
    }
    else
    ...
    if(saveflag)//
    {
        SpeedMonitor_SaveOneRecordData(stRunRecord,s_stSpeedAttrib.curTimeVal);
    }
    else
    {
...
unsigned char SpeedMonitor_IsOverSpeed(u8 CurrentSpeed, u8 MaxSpeed, u32 UpDelaySec)
{
    ...
    if(CurrentSpeed >= MaxSpeed)//&&SpeedMonitor_GetCurRunStatus())
    {
        ...
        {
             //liamtsen 2017-09-18
            //Report_UploadPositionInfo(CHANNEL_DATA_1);//超速的第1个点
...
unsigned char SpeedMonitor_GetCurSpeed(void)
{
    ...
    {
        CurrentSpeed = Pulse_GetSecondSpeed();
    }
    else
    {
...
void SpeedMonitor_CheckOverStopTime(void)
{
    ...
    if((CAR_STATUS_RUN==s_stSpeedAttrib.prerunStatus)&&(CAR_STATUS_STOP==s_stSpeedAttrib.runStatus))//由行驶到停驶
    {
        s_stSpeedAttrib.startStopTime = s_stSpeedAttrib.curTimeVal-RUN_CHAGE_STATUS_TIME;//
        SpeedMonitor_SaveStopStartTime();
    }
    else
...
///////////////////以下是为部标送检所添加///////////////////////
/**************************************************************************
//函数名：SpeedMonitor_OnceOverReport
...
//备注：重点是超速开始时刻，超速报警时刻
***************************************************************************/
static void SpeedMonitor_OverspeedOnceOverReport(void)
{
    u8	Speed;
    static u8 ReportFlag = 0;
	  ...
	  if(0 == DetectStatus(DetectOption_protocolTest))
		{
		    return ;
		}
	
	  Speed = SpeedMonitor_GetCurSpeed();
	  if((Speed >= s_stSpeedAttrib.curSpeedLimit)&&(0 == ReportFlag))
	  {
          ...
          else if(Speed < s_stSpeedAttrib.curSpeedLimit)
          {
                ReportFlag = 0;
          }
}
...
{
		SpeedMonitor_OverspeedOnceOverReport();//dxl,2016.3.21 部标协议检测时需添加这个函数调用，重点检测超速开始时刻和超速报警时刻
    //超速判断
...
***************************************************************************/
static void SpeedMonitor_OvertimeOnceRunReport(void)
{
	  static u8 count1 = 0;
	  static u8 count2 = 0;
	  ...
	  Speed = SpeedMonitor_GetCurSpeed();
	  if((Speed > 0)&&(Speed < s_stSpeedAttrib.curSpeedLimit)&&(0 == LastSpeed))
	  {
		    Report_UploadPositionInfo(CHANNEL_DATA_1|CHANNEL_DATA_2);
			  count1++;
			  if(count1 >= 2)
...
void SpeedMonitor_CheckForbidAlarm(void)
{
    if(0==s_stSpForbid.CheckVaild)return;
		SpeedMonitor_OvertimeOnceRunReport();//部标协议测试超时驾驶时调用此函数，重点测试超时驾驶开始时刻和报警时刻
...
void SpeedMonitor_ScanTask(void)
{
    ...
    ///////////////////////////
    LZM_PublicSetOnceTimer(&s_stSpeedTimer[SPEED_TIMER_CHECK],LZM_AT_ONCE,SpeedMonitor_CheckOverSpeedAlarm);     
}
/*************************************************************
...
** 调用模块:
*************************************************************/
void SpeedMonitor_CheckTimeChange(void)
{
    u32 count;
    count = RTC_GetCounter();
    s_stSpeedAttrib.checkTimeCnt++;
    if((s_stSpeedAttrib.curTimeVal != count)||(s_stSpeedAttrib.checkTimeCnt>6))
    {
        s_stSpeedAttrib.checkTimeCnt = 0;
        s_stSpeedAttrib.curTimeVal   = count;
        /////////////////
        LZM_PublicSetOnceTimer(&s_stSpeedTimer[SPEED_TIMER_CHECK],PUBLICSECS(0.1),SpeedMonitor_ScanTask);
    }
}
/*************************************************************
...
{
    s_stSpeedAttrib.curTimeVal = RTC_GetCounter();
    s_stSpeedAttrib.checkTimeCnt = 0;
    LZM_PublicSetCycTimer(&s_stSpeedTimer[SPEED_TIMER_TASK],PUBLICSECS(0.2),SpeedMonitor_CheckTimeChange);    
}
/*************************************************************
