...
typedef enum
{
	DetectOption_driverNoSignIn = 0,
	DetectOption_machine1,
	DetectOption_machine2,
	DetectOption_machine3,
	DetectOption_hyTest,
	DetectOption_functionTest,
	DetectOption_protocolTest,
	DetectOption_gbTest,
}E_DetectOption;
typedef union 
{
	...
	struct
	{
		u8	driverNoSignIn:1;						//bit0:为1是开启行驶时驾驶员未签到提醒
		u8	ministryTestMachine1:1;			//bit1:为1是部标协议检测1号机检测模式
		u8	ministryTestMachine2:1;			//bit2:为1是部标协议检测2号机检测模式
		u8	ministryTestMachine3:1;			//bit3:为1是部标协议检测3号机检测模式
		u8	hyTest:1;										//bit4:为1是全国货运平台检测模式
		u8	functionTest:1;							//bit5:为1是部标功能检测模式			808
		u8	protocolTest:1;							//bit6:为1是部标协议检测模式，当为协议检测时必须设置是几号机		794
		u8	gbTest:1;										//bit7:为1是国标检测模式
	}testBit;//为了方便仿真查看,可以不要这个结构体
}DetectControlDef;//过检控制
...
u8 SpeedMonitor_GetCurSpeedType(void);
...
//中断资源  :  
//返回      :0:开关未打开;1:开关打开
//备注      :DetectStatus_gbTest:为1是国标检测模式
//备注      :DetectStatus_protocolTest:为1是部标协议检测模式，当为协议检测时必须设置是几号机
//备注      :DetectStatus_functionTest:为1是部标功能检测模式
//备注      :DetectStatus_hyTest:为1是全国货运平台检测模式
//备注      :DetectStatus_machine3:为1是部标协议检测3号机检测模式
//备注      :DetectStatus_machine2:为1是部标协议检测2号机检测模式
//备注      :DetectStatus_machine1:为1是部标协议检测1号机检测模式
//备注      :DetectStatus_driverNoSignIn:为1是开启行驶时驾驶员未签到提醒
*********************************************************************/
